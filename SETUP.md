...
2. 次のプロパティ（列）を作成します（**名前を完全一致**させてください）：
   ...
   任意（作成すると自動的に記録されるようになります。作らなくてもエラーにはなりません）：
   ...
   | エリア | テキスト |
   | 概算金額 | 数値 |
   | ステータス | セレクト（※「ステータス」型ではなく「セレクト」型で作成すること） |
   | プライバシー同意 | チェックボックス |
   | 掲載同意 | チェックボックス |
   | 掲載同意の範囲 | テキスト |
   | ご紹介者 | テキスト |
   ...
   ※「ご紹介者」を作成しておくと、紹介制度（次回予約3,000円引き）を使ったお客様の紹介者名が
   自動で記録されます。次回予約時の割引適用自体は、この記録を見ながら手動で行ってください。
